# French translations for claude-fleet-starter installer.
# All strings are short paragraphs that get rendered with `say "$VAR"` or
# `explain "$VAR"`. printf-style %s placeholders are passed as arguments.
...
T_OS_DETECTED="✓ Système d'exploitation détecté : %s"
T_CHECKING_DEPS="Vérification des prérequis (curl, bash, etc.)..."
T_DEPS_MISSING="Il manque des outils de base. Lis le message d'erreur juste au-dessus pour savoir quoi installer."

# ── Claude Code ───────────────────────────────────────────────────────────────
T_CLAUDE_FOUND="✓ Claude Code est déjà installé (version : %s)"
...
T_CLAUDE_SKIPPED="OK, je n'installe pas Claude Code. Tu pourras le faire plus tard avec : curl -fsSL https://claude.ai/install.sh | bash"
...
T_INSTALLING_TAILSCALE="Installation de Tailscale en cours..."

T_TAILSCALE_AUTH_NEEDED="
Tailscale est installé, mais il faut maintenant le connecter à ton compte.

Copie cette commande, colle-la dans un terminal (peut-être le même que celui-ci),
...
T_INSTALLING_SKILL="• Installation de %s..."
...
T_MEMORY_INSTALLED="✓ Mémoires de démarrage installées dans ~/.claude/projects/.../memory/"
...
T_BOOTSTRAP_PATH_PROMPT="OK. Donne-moi le chemin du dossier (ex: /home/toi/Documents/claude-history) :"

T_BOOTSTRAP_PATH_INVALID="⚠ Le chemin %s n'existe pas, je passe."
...
Quand tu lanceras Claude Code la première fois, tu lui diras :
  ...
  Analyse mon historique dans %s et génère mon CLAUDE.md selon les
